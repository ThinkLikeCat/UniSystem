...
import { Plus, Search, Pencil, Trash2, BookMarked } from "lucide-react";
...
export default function SpecialtiesPage() {
  ...
  const [editItem, setEditItem] = useState<SpecialtyDto | null>(null);
  const [name, setName] = useState("");
  const [code, setCode] = useState("");
  const [maxDurationInYears, setMaxDurationInYears] = useState("");
  ...
  const openCreate = () => {
    ...
    setName("");
    setCode("");
    setMaxDurationInYears("");
    setError("");
    setModalOpen(true);
  };

  const openEdit = (item: SpecialtyDto) => {
    setEditItem(item);
    setName(item.name);
    setCode(item.code);
    setMaxDurationInYears(String(item.maxDurationInYears));
    setError("");
    setModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const duration = parseInt(maxDurationInYears, 10);
    if (!duration || duration < 1) {
      setError("Укажите корректную длительность (лет)");
      return;
    }
    setSaving(true);
    try {
      const data = { name: name.trim(), code: code.trim(), maxDurationInYears: duration };
      if (editItem) {
        await updateSpecialty(editItem.id, data);
      } else {
  ...
  const handleDelete = async (item: SpecialtyDto) => {
    if (!confirm(`Удалить специальность "${item.name}"?`)) return;
    try {
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Input
                  ...
                  type="number"
                  value={maxDurationInYears}
                  onChange={(e) => setMaxDurationInYears(e.target.value)}
                  placeholder="4"
                />
              </div>
              {error && (
