...
export default function DepartmentsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        await updateDepartment(editItem.id, name.trim());
      } else {
  ...
  const handleDelete = async (item: DepartmentDto) => {
    if (!confirm(`Удалить кафедру "${item.name}"?`)) return;
    try {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Кафедры</h1>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          ...
          >
            <h2 className="text-lg font-semibold text-text-primary mb-4">
              {editItem ? "Редактировать кафедру" : "Добавить кафедру"}
            </h2>
