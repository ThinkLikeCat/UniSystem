...
import { Plus, Search, Pencil, Trash2, BookCheck } from "lucide-react";

export default function DocumentStatusesPage() {
  const [items, setItems] = useState<DocumentStatusDto[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");
  const [modalOpen, setModalOpen] = useState(false);
  const [editItem, setEditItem] = useState<DocumentStatusDto | null>(null);
  ...
  const openEdit = (item: DocumentStatusDto) => {
    setEditItem(item);
    setName(item.name);
    setError("");
    setModalOpen(true);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        await updateDocumentStatus(editItem.id, name.trim());
      } else {
  ...
  const handleDelete = async (item: DocumentStatusDto) => {
    if (!confirm(`Удалить статус "${item.name}"?`)) return;
    try {
      await deleteDocumentStatus(item.id);
      await fetchItems();
    } catch {
      ...
      >
        {loading ? (
          ...
          <div className="flex flex-col items-center py-12 text-text-muted">
            <BookCheck className="w-12 h-12 mb-3 opacity-50" />
